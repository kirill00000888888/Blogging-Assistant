...
- `TG_HF_TIMEOUT_MS` - отдельный таймаут Hugging Face для Telegram. По умолчанию бот ждет не меньше `180000` мс.
- `TG_HF_MAX_TOKENS` - отдельная длина ответа для Telegram. По умолчанию компактнее веб-версии, чтобы реже ловить таймауты.
...
- Если таймауты появляются только в Telegram, увеличьте `TG_HF_TIMEOUT_MS`, уменьшите `TG_HF_MAX_TOKENS` или попросите бота отвечать короче.
